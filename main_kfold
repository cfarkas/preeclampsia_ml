...
###############################################################################
def parse_arguments():
    p = argparse.ArgumentParser(description="Deterministic ML pipeline (10‑fold CV).")
...
def main():
    ...
    # ------------------------------------------------------------------ #
    # 7.  CLASSIFICATION – 10‑fold STRATIFIED CV -------------------------
    # ------------------------------------------------------------------ #
    skf = StratifiedKFold(n_splits=10, shuffle=True, random_state=SEED)

    # encode object features once
    ...
    for out in classification_out:
        ...
        for m in meth_cls:
            pipe = make_pipeline(StandardScaler(), classifiers[m])

            # macro‑recall (mean over folds)
            scores = cross_val_score(pipe, X_all, y,
                                     cv=skf, scoring='recall_macro')
            recall_df.loc[out, m] = scores.mean()

            # cross‑validated predictions for confusion matrix
            y_pred = cross_val_predict(pipe, X_all, y, cv=skf)
            conf_mats[out][m] = confusion_matrix(y, y_pred)

            # fit on full data once for permutation importance
            pipe.fit(X_all, y)
            try:
                p_res = permutation_importance(
                    pipe, X_all, y, n_repeats=5,
    ...
    for out in continuous_out:
        ...
        for m in meth_reg:
            pipe = make_pipeline(StandardScaler(), regressors[m])

            mse_scores = -cross_val_score(pipe, X_all, y,
                                          cv=kf, scoring='neg_mean_squared_error')
            rmse_scores = np.sqrt(mse_scores)
            mae_scores = -cross_val_score(pipe, X_all, y,
                                          cv=kf, scoring='neg_mean_absolute_error')
            r2_scores  =  cross_val_score(pipe, X_all, y, cv=kf, scoring='r2')

            reg_metrics[out][m] = (mse_scores.mean(),
                                   rmse_scores.mean(),
                                   mae_scores.mean(),
                                   r2_scores.mean())
    ...
    # ------------------------------------------------------------------ #
    # 11. FIGURE 3 – best‑performing classifier per outcome 
    # ------------------------------------------------------------------ #
    ordered_panels = classification_out
    best_method = {o: recall_df.loc[o].idxmax() for o in ordered_panels}
    ...
    for idx, out in enumerate(ordered_panels):
        ...
        axes3[idx].set_title(f"{chr(65+idx)}) {out.replace('_',' ').title()} – {m}",
                             fontsize=18)
    ...
    # ------------------------------------------------------------------ #
    # 12. IMPORTANCES DOT‑PLOT / 13. RECALL HEAT‑MAP 
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # 14. REGRESSION METRICS & 15. FEATURE CSV EXPORT 
    # ------------------------------------------------------------------ #
    with open(f"{args.output}/regression_metrics_summary.txt", "w") as f:
        f.write("Regression Metrics (mean of 5‑fold CV)\n\n")
        for out in continuous_out:
            f.write(f"{out}:\n")
            for m in meth_reg:
